...
def load_corpus(path, word2id, max_sen_len=50):
    """
    :param path: 样本语料库的文件夹；如训练样本路径为：./data/train/
    :return: 文本内容contents，以及分类标签labels(onehot形式)
    """
    _path = [os.path.join(path, w) for w in os.listdir(path)]
    contents, labels = [], []
    for p in _path:
        with open(p, encoding='utf-8') as f:
            for line in f.readlines():
                sp = line.strip().split()
                label = cat_to_id()[sp[0]]
    ...
    labels = kr.utils.to_categorical(labels, len(cat_to_id()))
...
def build_word2id(file):
    ...
    """
    word2id = {'_PAD_': 0}
    train_path = [os.path.join('./data/train/', w) for w in os.listdir('./data/train/')]
    dev_path = [os.path.join('./data/dev/', w) for w in os.listdir('./data/dev/')]
    test_path = [os.path.join('./data/test/', w) for w in os.listdir('./data/test/')]
    path = train_path + dev_path + test_path
...
if __name__=='__main__':
    ...
    print('加载train语料库========================')
    train = load_corpus('./data/train/', word2id, max_sen_len=100)
    print('加载dev语料库==========================')
    dev = load_corpus('./data/dev/', word2id, max_sen_len=100)
    print('加载test语料库=========================')
    test = load_corpus('./data/test/', word2id, max_sen_len=100)
